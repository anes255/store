        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 sm:py-5 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 sm:gap-3 shrink-0">
            ...
            <Link to={`/s/${storeSlug}/favorites`} className="p-1.5 sm:p-2 hover:bg-white/20 rounded-full relative">
              ...
              {wishlistStore.count()>0&&<span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none pointer-events-none" style={{width:20,height:20,minWidth:20,minHeight:20}}>{wishlistStore.count()}</span>}
            </Link>
            <button onClick={()=>setCartOpen(true)} className="p-1.5 sm:p-2 hover:bg-white/20 rounded-full relative">
              <ShoppingCart size={18} className="sm:w-5 sm:h-5"/>
              {getCount()>0&&<span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none pointer-events-none" style={{width:20,height:20,minWidth:20,minHeight:20}}>{getCount()}</span>}
            </button>
          </div>
        </div>
        {/* Mobile search */}
              ...
              {allImages.length > 1 && (
                <>
                  <button onClick={(e) => { e.stopPropagation(); setSelectedImage((selectedImage - 1 + allImages.length) % allImages.length); }}
                    className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/70 text-white grid place-items-center z-10 active:scale-90 shadow-lg border border-white/20">
                    <ArrowLeft size={18}/>
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); setSelectedImage((selectedImage + 1) % allImages.length); }}
                    className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/70 text-white grid place-items-center z-10 active:scale-90 shadow-lg border border-white/20">
                    <ArrowRight size={18}/>
                  </button>
                </>
              )}
