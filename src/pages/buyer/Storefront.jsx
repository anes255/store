    ...
    <div className={`storefront-scope min-h-screen pb-20 md:pb-0 ${buyerTheme.mode === 'dark' ? 'buyer-theme-dark bg-[#0b1020] text-gray-100' : 'bg-[#f5f5f5] text-gray-900'}`} style={bodyTextColor?{color:bodyTextColor}:undefined}>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 sm:py-5 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Link to={`/s/${storeSlug}`} className="flex items-center gap-2 sm:gap-4 min-w-0 flex-shrink" style={{color:headerText}}>
              ...
              <span data-store-name className="store-header-title text-base sm:text-2xl font-extrabold truncate" style={{color:headerText,WebkitTextFillColor:headerText,fontFamily:nameFont}}>{store.name}</span>
            </Link>
          </div>
      ...
      {store.whatsapp_number && (
        <a href={`https://wa.me/${store.whatsapp_number.replace(/[^0-9]/g,'')}`} target="_blank" rel="noopener noreferrer"
          className="fixed z-40 w-14 h-14 rounded-full flex items-center justify-center shadow-xl hover:scale-110 transition-transform animate-pulse bottom-[calc(170px+env(safe-area-inset-bottom))] md:bottom-[160px] right-4 md:right-5"
      ...
      {/* ============ WHATSAPP FLOATING BUTTON ============ */}
      {store.whatsapp_button_enabled && store.whatsapp_button_number && (() => {
        const cleanNum = String(store.whatsapp_button_number).replace(/[^\d+]/g,'').replace(/^\+/,'');
        const msg = encodeURIComponent(store.whatsapp_button_message || `Hi ${store.name}, I have a question.`);
        return (
          <a
