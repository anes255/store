          ...
          {items.map((it, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {it.variant && <p className="text-[11px] text-gray-400">{it.variant.label || (it.variant.type === 'color' && it.variant.value && /^#[0-9a-f]{3,8}$/i.test(it.variant.value) ? it.variant.name : it.variant.value) || it.variant.name || ''}</p>}
    ...
    <div className={`storefront-scope min-h-screen pb-20 md:pb-0 ${buyerTheme.mode === 'dark' ? 'buyer-theme-dark bg-[#0b1020] text-gray-100' : 'bg-[#f5f5f5] text-gray-900'}`} style={bodyTextColor?{color:bodyTextColor}:undefined}>
      ...
      <footer className="bg-white border-t border-gray-100 py-8 px-4 mt-8">
        <div className="max-w-7xl mx-auto text-center">
          ...
          {(store.facebook_url || store.instagram_url || store.twitter_url || store.tiktok_url || store.youtube_url || store.linkedin_url) && (
            <div className="flex items-center justify-center gap-4 mt-4">
              {store.facebook_url && <a href={store.facebook_url} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-600 transition-colors"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>}
      ...
      {(store.whatsapp_button_enabled && store.whatsapp_button_number || store.whatsapp_number) && (() => {
        const rawNum = store.whatsapp_button_enabled && store.whatsapp_button_number ? store.whatsapp_button_number : store.whatsapp_number;
