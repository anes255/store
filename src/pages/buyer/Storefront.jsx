    ...
    <div className={`storefront-scope min-h-screen pb-20 md:pb-0 ${buyerTheme.mode === 'dark' ? 'buyer-theme-dark bg-[#0b1020] text-gray-100' : 'bg-[#f5f5f5] text-gray-900'}`} style={bodyTextColor?{color:bodyTextColor}:undefined}>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 sm:py-5 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 sm:gap-3 shrink-0 overflow-x-auto max-w-full" style={{scrollbarWidth:'none'}}>
            ...
            <Link to={`/s/${storeSlug}/favorites`} className="p-1.5 sm:p-2 hover:bg-white/20 rounded-full relative shrink-0" title="Favorites">
              ...
              {wishlist.length>0&&<span className="absolute -top-1 -right-1 w-5 h-5 min-w-[20px] min-h-[20px] bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none">{wishlist.length}</span>}
            </Link>
            <button onClick={()=>setCartOpen(true)} className="p-1.5 sm:p-2 hover:bg-white/20 rounded-full relative shrink-0">
              <ShoppingCart size={18} className="sm:w-5 sm:h-5"/>
              {getCount()>0&&<span className="absolute -top-1 -right-1 w-5 h-5 min-w-[20px] min-h-[20px] bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none">{getCount()}</span>}
            </button>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          {(() => { const fb = store.facebook_url || store.social_facebook; const ig = store.instagram_url || store.social_instagram; const tw = store.twitter_url || store.social_twitter || store.twitter; const tk = store.tiktok_url || store.social_tiktok; const yt = store.youtube_url || store.youtube; const li = store.linkedin_url || store.social_linkedin || store.linkedin; const sn = store.snapchat || store.social_snapchat; const pin = store.pinterest || store.social_pinterest; return (fb||ig||tw||tk||yt||li||sn||pin) ? (
            <div className="flex items-center justify-center gap-4 mt-4">
              ...
              {sn && <a href={sn} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-yellow-400 transition-colors"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12.206.793c.99 0 4.347.276 5.93 3.821.529 1.193.403 3.219.299 4.847l-.003.06c-.012.18-.022.345-.03.51.075.045.203.09.401.09.3-.016.659-.12 1.033-.301.165-.088.344-.104.464-.104.182 0 .359.029.509.09.45.149.734.479.734.838.015.449-.39.839-1.213 1.168-.089.029-.209.075-.344.119-.45.135-1.139.36-1.333.81-.09.224-.061.524.12.868l.015.015c.06.136 1.526 3.475 4.791 4.014.255.044.435.27.42.509 0 .075-.015.149-.045.225-.24.569-1.273.988-3.146 1.271-.059.091-.12.375-.164.57-.029.179-.074.36-.134.553-.076.271-.27.405-.555.405h-.03c-.135 0-.313-.031-.538-.076-.375-.09-.84-.181-1.455-.181-.592 0-1.119.09-1.559.254-.645.24-1.17.57-1.564.886-.6.45-1.036.756-1.564.756h-.12c-.569 0-1.004-.306-1.604-.771-.39-.313-.916-.645-1.549-.885-.435-.165-.96-.254-1.559-.254-.659 0-1.139.105-1.499.195-.195.044-.361.073-.495.073h-.045c-.239 0-.42-.121-.51-.404-.063-.195-.105-.376-.136-.569-.044-.181-.105-.465-.164-.556-1.874-.284-2.906-.702-3.146-1.271-.03-.075-.046-.15-.046-.226-.014-.24.166-.465.42-.509 3.264-.54 4.73-3.879 4.791-4.02l.016-.029c.18-.345.224-.645.119-.869-.195-.434-.884-.658-1.332-.809-.135-.045-.255-.09-.359-.12-.734-.284-1.199-.674-1.199-1.093-.016-.449.359-.839.809-.988.15-.06.329-.09.508-.09.12 0 .301.015.449.09.374.18.72.3 1.034.3.21 0 .345-.045.419-.09-.015-.18-.029-.359-.044-.539l-.003-.06c-.104-1.627-.229-3.654.3-4.847C7.857 1.069 11.247.793 12.207.793"/></svg></a>}
              {pin && <a href={pin} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-red-500 transition-colors"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738.098.119.112.224.083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.631-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24 12.017 24c6.624 0 11.99-5.367 11.99-11.988C24.007 5.367 18.641 0 12.017 0z"/></svg></a>}
            </div>
          ) : null; })()}
        </div>
      </footer>
      ...
      <div className={`md:hidden fixed bottom-0 left-0 right-0 z-30 px-2 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] flex items-stretch justify-around gap-1 shadow-[0_-4px_16px_rgba(0,0,0,0.08)] ${buyerTheme.mode==='dark'?'bg-gray-900 border-t border-white/10':'bg-white border-t border-gray-100'}`}>
        ...
        <Link to={`/s/${storeSlug}/favorites`} className="flex-1 min-w-0 flex flex-col items-center gap-0.5 py-1.5 rounded-xl text-gray-400 dark:text-gray-500 active:bg-gray-100 dark:active:bg-white/10 relative">
          <Heart size={20}/>
          {wishlist.length>0&&<span className="absolute top-0 right-1/4 w-5 h-5 min-w-[20px] min-h-[20px] bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none">{wishlist.length}</span>}
        ...
        <button onClick={()=>setCartOpen(true)} className="flex-1 min-w-0 flex flex-col items-center gap-0.5 py-1.5 rounded-xl text-gray-400 dark:text-gray-500 active:bg-gray-100 dark:active:bg-white/10 relative">
          ...
          {getCount()>0&&<span className="absolute top-0 right-1/4 w-5 h-5 min-w-[20px] min-h-[20px] bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none">{getCount()}</span>}
