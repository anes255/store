      ...
      <div className="mt-2">
        ...
        {unitCount===1?(
          <div className="flex flex-wrap items-center gap-2">
            {types.map(type=>{const group=groups[type];const uv=unitVars[0]||{};const sel=uv[type];return(
              <div key={type} className="flex items-center gap-1.5">
                {type==='color'?group.map(v=>{const isSel=sel===v._idx;const colorVal=v.value||'#ccc';const useColor=isColorValue(colorVal);const hasImg=v.images&&v.images.length>0;return(
                  ...
                  <button key={v._idx} onClick={()=>setLpUnitVariant(pid,0,type,v._idx)}
          ...
          <div className="grid gap-1" style={{gridTemplateColumns:`repeat(${Math.min(unitCount,3)}, 1fr)`}}>
            {Array.from({length:unitCount},(_,ui)=>{const uv=unitVars[ui]||{};return(
              <div key={ui} className="flex items-center gap-1.5 rounded-lg px-1.5 py-1 bg-gray-50 border border-gray-200">
                ...
                <div className="flex items-center gap-1 flex-wrap">
                  {types.map(type=>{const group=groups[type];const sel=uv[type];return type==='color'?group.map(v=>{const isSel=sel===v._idx;const colorVal=v.value||'#ccc';const useColor=isColorValue(colorVal);const hasImg=v.images&&v.images.length>0;return(
                    ...
                    <button key={v._idx} onClick={()=>setLpUnitVariant(pid,ui,type,v._idx)}
                      className={`px-1.5 py-0.5 rounded text-[9px] font-bold border transition-all ${isSel?'text-white':'border-gray-300 text-gray-500 hover:border-gray-400'}`}
                      style={isSel?{backgroundColor:pc,borderColor:pc}:{}}>
                      {v.name||v.value||t('store.variantOption','Option')}
                    </button>
                  );})})}
