...
function SiteSettings(){
  ...
  const imgUp=(field)=>(e)=>{const f=e.target.files?.[0];if(!f){toast.error('No file');return;}const reader=new FileReader();reader.onerror=()=>toast.error('Read failed');reader.onload=()=>{const img=new Image();img.onerror=()=>toast.error('Image invalid');img.onload=()=>{try{const c=document.createElement('canvas');const max=field==='favicon'?128:400;const ratio=Math.min(max/img.width,max/img.height,1);c.width=Math.max(1,Math.round(img.width*ratio));c.height=Math.max(1,Math.round(img.height*ratio));c.getContext('2d').drawImage(img,0,0,c.width,c.height);const compressed=c.toDataURL('image/png');setS(prev=>({...(prev||{}),[field]:compressed}));toast.success('Uploaded — click Save to apply');}catch(err){toast.error('Compress failed');}};img.src=reader.result;};reader.readAsDataURL(f);e.target.value='';};
  return(<div>
