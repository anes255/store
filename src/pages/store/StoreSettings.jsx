...
export default function StoreSettings(){
  ...
  const seedW=async()=>{try{await api.post(`/manage/stores/${currentStore.id}/shipping-wilayas/seed`);toast.success(t('storePage.wilayasLoaded','58 wilayas loaded!'));loadData();}catch{toast.error(t('storePage.failed','Failed'));}};
  const addStaff=async()=>{try{await api.post(`/owner/stores/${currentStore.id}/staff`,sf);toast.success(t('storePage.added','Added!'));setShowStaff(false);setSf({name:'',email:'',phone:'',password:'',role:'viewer'});loadData();}catch{toast.error(t('storePage.failed','Failed'));}};
...
<button onClick={()=>window.location.href='/dashboard/page-builder'} className="w-full p-5 bg-gradient-to-br from-violet-50 to-pink-50 rounded-xl flex items-center justify-between hover:ring-2 hover:ring-violet-200 transition-all text-left">
  <div className="flex items-center gap-3 min-w-0">
    <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center shrink-0 shadow-sm"><Sparkles size={20} className="text-violet-600"/></div>
    <div className="min-w-0"><h4 className="font-bold text-sm text-gray-900">Section Animations</h4><p className="text-[11px] text-gray-400 mt-0.5">Moved to the Page Builder → Motion button. Pick how sections reveal as buyers scroll.</p></div>
  </div>
  <ExternalLink size={16} className="text-violet-500 shrink-0 ml-2"/>
</button>
...
{sec==='users'&&<><div className="glass-card-solid p-4 sm:p-6"><div className="flex items-center justify-between mb-4"><div className="flex gap-4"><span className="text-sm font-bold text-brand-600 border-b-2 border-brand-500 pb-2">Team Members</span></div><button onClick={()=>setShowStaff(true)} className="btn-primary text-xs flex items-center gap-1"><Plus size={14}/>Create New User</button></div>
<table className="w-full text-sm"><thead><tr className="text-left text-xs text-gray-400 uppercase"><th className="pb-2">User</th><th className="pb-2">Role</th><th className="pb-2">Actions</th></tr></thead><tbody>{staff.map(x=>(<tr key={x.id} className="border-t border-gray-100"><td className="py-3"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-brand-100 flex items-center justify-center text-brand-600 font-bold text-sm">{x.name?.[0]?.toUpperCase()}</div><div><p className="font-semibold text-gray-800">{x.name}</p><p className="text-xs text-gray-400">{x.email}</p></div></div></td><td><select className="input-field !py-1.5 !w-36 text-sm" defaultValue={x.role}>{Object.entries(roles).map(([k,r])=>(<option key={k} value={k}>{r.label}</option>))}{platformTemplates.map(tpl=>{const lang=(document.documentElement.lang||'en').slice(0,2);const name=(tpl.name&&(tpl.name[lang]||tpl.name.en))||'Role';return(<option key={'tpl_'+tpl.id} value={'tpl_'+tpl.id}>{name}</option>);})}</select></td><td><div className="flex gap-1"><button className="p-1.5 hover:bg-gray-100 rounded text-gray-400"><Eye size={14}/></button><button className="p-1.5 hover:bg-red-50 rounded text-red-400"><Trash2 size={14}/></button></div></td></tr>))}</tbody></table>{staff.length===0&&<p className="text-center py-8 text-gray-400 text-sm">No team members yet</p>}</div>
<div className="glass-card-solid p-4 sm:p-6"><h3 className="font-bold text-gray-900 mb-4">Roles & Permissions</h3><div className="space-y-3">{Object.entries(roles).map(([k,r])=>(<div key={k} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl"><div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-xl ${r.color} flex items-center justify-center text-white font-bold text-sm`}>{r.label[0]}</div><div><p className="font-bold text-gray-800">{r.label} <span className="badge badge-neutral text-[9px] ml-1">SYSTEM</span></p><p className="text-xs text-gray-400">{r.desc}</p></div></div><button onClick={()=>setShowRole(k)} className="text-sm text-brand-600 font-medium hover:underline">View</button></div>))}{platformTemplates.map(tpl=>{const lang=(document.documentElement.lang||'en').slice(0,2);const name=(tpl.name&&(tpl.name[lang]||tpl.name.en))||'Role';const desc=(tpl.description&&(tpl.description[lang]||tpl.description.en))||`Platform template · ${(tpl.permissions||[]).length} permissions`;return(<div key={'tpl_'+tpl.id} className="flex items-center justify-between p-4 bg-purple-50 rounded-xl"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-purple-500 flex items-center justify-center text-white font-bold text-sm">{name[0]?.toUpperCase()}</div><div><p className="font-bold text-gray-800">{name} <span className="badge badge-neutral text-[9px] ml-1 bg-purple-100 text-purple-700">PLATFORM</span></p><p className="text-xs text-gray-400">{desc}</p></div></div><span className="text-xs text-purple-500 font-medium">{(tpl.permissions||[]).length} perms</span></div>);})}</div></div></>}

{sec==='checkout'&&<><SH icon={ShoppingCart} title="Purchase Flow" subtitle="Optimize visitor to customer conversion." accent="purple"/>
<div className="glass-card-solid p-4 sm:p-6 space-y-3 mt-4"><h3 className="font-bold flex items-center gap-2"><Lock size={16}/>Data Collection</h3><div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl"><div><p className="font-semibold text-sm">Full Name</p><p className="text-[10px] text-gray-400">Primary identifier</p></div><span className="badge badge-danger text-[10px]">REQUIRED</span></div><div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl"><div><p className="font-semibold text-sm">Phone Number</p><p className="text-[10px] text-gray-400">Delivery coordination</p></div><span className="badge badge-danger text-[10px]">REQUIRED</span></div><T label="Email Collection" desc="Automated invoice flow." checked={s.checkout_email} onChange={e=>setV('checkout_email',e.target.checked)}/></div>
