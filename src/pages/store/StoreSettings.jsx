...
<div className="glass-card-solid p-4 sm:p-6 space-y-3"><div className="flex items-center gap-2"><Palette size={16} className="text-brand-500"/><div><h4 className="font-bold text-sm">{t('storePage.dashboardButtonColor','Dashboard Button Color')}</h4><p className="text-[11px] text-gray-400">{t('storePage.dashboardButtonColorDesc','Color of the action buttons (Save, Add…) in your admin dashboard.')}</p></div></div>
<div className="flex flex-wrap gap-2">{['#C5A55A','#7C3AED','#3B82F6','#10B981','#06B6D4','#14B8A6','#F59E0B','#F97316','#EF4444','#EC4899','#111827'].map(c=>(<button key={c} type="button" onClick={()=>adminTheme.setButtonColor(c)} className={`w-8 h-8 rounded-full transition-transform hover:scale-110 ${(adminTheme.buttonColor||'').toLowerCase()===c.toLowerCase()?'ring-2 ring-offset-2 ring-brand-500 scale-110':''}`} style={{backgroundColor:c}}/>))}</div>
<div className="flex items-center gap-3"><input type="color" value={adminTheme.buttonColor||'#C5A55A'} onChange={e=>adminTheme.setButtonColor(e.target.value)} className="w-8 h-8 rounded-lg cursor-pointer border-0 p-0" style={{appearance:'none'}}/><input className="input-field !w-32 text-sm font-mono" defaultValue={adminTheme.buttonColor||'#C5A55A'} onChange={e=>{const v=e.target.value.trim();if(/^#[0-9A-Fa-f]{6}$/.test(v))adminTheme.setButtonColor(v);}}/><button type="button" className="px-4 py-2 rounded-xl text-white text-xs font-bold shadow-lg" style={{backgroundColor:adminTheme.buttonColor||'#C5A55A'}}>{t('storePage.saveChanges','Save')}</button></div></div>
