...
<table className="w-full text-sm"><thead><tr className="text-left text-xs text-gray-400 uppercase"><th className="pb-2">User</th><th className="pb-2">Role</th><th className="pb-2">Actions</th></tr></thead><tbody>{staff.map(x=>(<tr key={x.id} className="border-t border-gray-100"><td className="py-3"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-brand-100 flex items-center justify-center text-brand-600 font-bold text-sm">{x.name?.[0]?.toUpperCase()}</div><div><p className="font-semibold text-gray-800">{x.name}</p><p className="text-xs text-gray-400">{x.email}</p></div></div></td><td><select className="input-field !py-1.5 !w-36 text-sm" defaultValue={x.role}>{platformTemplates.map(tpl=>{const lang=(document.documentElement.lang||'en').slice(0,2);const name=(tpl.name&&(tpl.name[lang]||tpl.name.en))||'Role';return(<option key={'tpl_'+tpl.id} value={'tpl_'+tpl.id}>{name}</option>);})}</select></td><td><div className="flex gap-1"><button className="p-1.5 hover:bg-gray-100 rounded text-gray-400"><Eye size={14}/></button><button className="p-1.5 hover:bg-red-50 rounded text-red-400"><Trash2 size={14}/></button></div></td></tr>))}</tbody></table>{staff.length===0&&<p className="text-center py-8 text-gray-400 text-sm">No team members yet</p>}</div>
<div className="glass-card-solid p-4 sm:p-6"><h3 className={`font-bold mb-4 ${settingsIsDark?'text-gray-100':'text-gray-900'}`}>Roles & Permissions</h3><div className="space-y-3">{platformTemplates.length===0&&<p className={`text-sm ${settingsIsDark?'text-gray-400':'text-gray-500'}`}>No roles defined yet. Ask your platform admin to add role templates.</p>}{platformTemplates.map(tpl=>{const lang=(document.documentElement.lang||'en').slice(0,2);const name=(tpl.name&&(tpl.name[lang]||tpl.name.en))||'Role';const desc=(tpl.description&&(tpl.description[lang]||tpl.description.en))||`${(tpl.permissions||[]).length} permissions`;return(<div key={'tpl_'+tpl.id} className={`flex items-center justify-between p-4 rounded-xl ${settingsIsDark?'bg-gray-800 border border-gray-700':'bg-gray-50'}`}><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-purple-500 flex items-center justify-center text-white font-bold text-sm">{name[0]?.toUpperCase()}</div><div><p className={`font-bold ${settingsIsDark?'text-gray-100':'text-gray-800'}`}>{name} <span className={`text-[9px] ml-1 px-1.5 py-0.5 rounded ${settingsIsDark?'bg-purple-500/20 text-purple-200':'bg-purple-100 text-purple-700'}`}>PLATFORM</span></p><p className={`text-xs ${settingsIsDark?'text-gray-400':'text-gray-400'}`}>{desc}</p></div></div><span className={`text-xs font-medium ${settingsIsDark?'text-purple-300':'text-purple-500'}`}>{(tpl.permissions||[]).length} perms</span></div>);})}</div></div></>}
