...
<table className="w-full text-sm"><thead><tr className="text-left text-xs text-gray-400 uppercase"><th className="pb-2">User</th><th className="pb-2">Role</th><th className="pb-2">Actions</th></tr></thead><tbody>{staff.map(x=>{const cur=x.role_template_id||x.role||'';return(<tr key={x.id} className="border-t border-gray-100"><td className="py-3"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-brand-100 flex items-center justify-center text-brand-600 font-bold text-sm">{x.name?.[0]?.toUpperCase()}</div><div><p className="font-semibold text-gray-800">{x.name}</p><p className="text-xs text-gray-400">{x.email}</p></div></div></td><td><select className="input-field !py-1.5 !w-48 text-sm" value={cur} onChange={e=>changeStaffRole(x.id,e.target.value)}><option value="">— Select role —</option><optgroup label="Platform Roles">{platformTemplates.map(tpl=>{const lang=(document.documentElement.lang||'en').slice(0,2);const name=(tpl.name&&(tpl.name[lang]||tpl.name.en))||'Role';return(<option key={'tpl_'+tpl.id} value={'tpl_'+tpl.id}>{name}</option>);})}</optgroup>{storeRoles.length>0&&<optgroup label="My Store Roles">{storeRoles.map(tpl=><option key={tpl.id} value={tpl.id}>{tpl.name}</option>)}</optgroup>}</select></td><td><div className="flex gap-1"><button onClick={()=>removeStaff(x.id)} className="p-1.5 hover:bg-red-50 rounded text-red-400"><Trash2 size={14}/></button></div></td></tr>);})}</tbody></table>{staff.length===0&&<p className="text-center py-8 text-gray-400 text-sm">No team members yet</p>}</div>
