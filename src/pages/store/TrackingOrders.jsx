        ...
        <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e=>e.stopPropagation()}>
          ...
          <div className="sticky top-0 bg-white z-10 px-6 pt-6 pb-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">{selectedOrder.company_name||'No delivery company'} {selectedOrder.tracking_number&&<span className="font-mono bg-gray-100 px-2 py-0.5 rounded ml-1">#{selectedOrder.tracking_number}</span>}</p>
            </div>
            <div className="flex items-center gap-2">
              {selectedOrder.tracking_number&&<button onClick={refreshTracking} disabled={trackingLoading} className="btn-ghost text-xs flex items-center gap-1"><RefreshCw size={12} className={trackingLoading?'animate-spin':''}/>Refresh</button>}
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 bg-gray-50 rounded-xl"><p className="text-[9px] font-bold text-gray-400 uppercase mb-1">Customer</p><p className="font-bold text-sm text-gray-900">{selectedOrder.customer_name}</p><p className="text-xs text-gray-500">{selectedOrder.customer_phone}</p></div>
              <div className="p-3 bg-gray-50 rounded-xl"><p className="text-[9px] font-bold text-gray-400 uppercase mb-1">Destination</p><p className="text-sm text-gray-700">{selectedOrder.shipping_wilaya||'—'}</p></div>
              <div className="p-3 bg-gray-50 rounded-xl"><p className="text-[9px] font-bold text-gray-400 uppercase mb-1">Partner</p><p className="font-bold text-sm text-gray-900">{selectedOrder.company_name||'N/A'}</p>{trackingData?.has_api&&<span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">LIVE</span>}</div>
            </div>

            {!selectedOrder.tracking_number?(
              <div className="p-8 text-center"><AlertTriangle size={36} className="mx-auto text-amber-400 mb-3"/><p className="font-bold text-gray-700">No tracking number</p><p className="text-xs text-gray-400 mt-1">Assign one from the Orders page</p></div>
              ...
              <>
                ...
                <div className="p-6 bg-gradient-to-br from-gray-50 to-white rounded-2xl border border-gray-100">
                  <h3 className="font-bold text-gray-800 mb-6">Delivery Progress</h3>
                  {isFailed?(
                      ...
                      <div className="relative flex justify-between">
                        {STEPS.map((step,i)=>{const Icon=step.icon;const done=i<=stepIdx;const active=i===stepIdx;return(
                          <div key={step.key} className="flex flex-col items-center" style={{width:`${100/STEPS.length}%`}}>
                            ...
                            <p className={`text-[10px] font-bold mt-2.5 text-center ${active?'text-brand-600':done?'text-emerald-600':'text-gray-400'}`}>{step.label}</p>
                          </div>
                        );})}
                      </div>
                    </div>
                ...
                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex-1"><p className="font-bold text-sm text-gray-900 capitalize">{trackingData?.raw_status||currentStatus.replace(/_/g,' ')}</p><p className="text-xs text-gray-500">via {trackingData?.company||selectedOrder.company_name}</p></div>
                ...
                {trackingData?.tracking_url&&!trackingData?.has_api&&(
                  <a href={trackingData.tracking_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl hover:bg-blue-100 transition-colors">
                    <ExternalLink size={16} className="text-blue-500"/><p className="text-sm font-bold text-blue-700 flex-1">Track on {selectedOrder.company_name}</p><ChevronRight size={14} className="text-blue-400"/>
                  </a>
                )}
                ...
                {trackingData?.history&&trackingData.history.length>0&&(
                  <div>
                    <h3 className="font-bold text-sm text-gray-800 mb-3">Timeline</h3>
