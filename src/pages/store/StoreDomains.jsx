...
export default function StoreDomains() {
  ...
  const addDomain = async () => {
    if (!domainName) return toast.error('Enter your domain');
    const clean = domainName.replace(/^https?:\/\//, '').replace(/\/.*$/, '').trim().toLowerCase();
    if (!clean.includes('.')) return toast.error('Enter a valid domain (e.g. mystore.com)');
    setAdding(true);
    try {
      await ownerApi.requestDomain(currentStore.id, { domain_name: clean });
      toast.success('Domain added! Configure your DNS to connect it.');
  ...
  const setActiveDomain = async (value) => {
    try {
      ...
      setActiveUrl(value);
      toast.success('Active URL updated');
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="page-header">Domains & URLs</h1><p className="text-sm text-gray-500 mt-1">Connect your own domain or use the platform URL</p></div>
        <button onClick={() => setShowModal(true)} className="btn-primary flex items-center gap-2 text-sm"><Plus size={16} />Connect Domain</button>
      </div>

      {/* Active URL Selection */}
      <div className="glass-card-solid p-6 mb-6">
        <h3 className="font-bold text-gray-900 mb-4">Active Store URL</h3>
        <p className="text-sm text-gray-500 mb-4">Choose which URL your customers will use</p>
      ...
      {domains.length > 0 && (
        <div className="glass-card-solid p-6 mb-6">
          <h3 className="font-bold text-gray-900 mb-4">Connected Domains</h3>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 ${d.status === 'active' ? 'bg-emerald-100 text-emerald-700' : d.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                        <StatusIcon size={10} />{d.status === 'pending' ? 'DNS not verified' : d.status}
                      </span>
                    </div>
                  </div>
                </div>
      ...
      <div className="glass-card-solid p-6">
        <h3 className="font-bold text-gray-900 mb-3 flex items-center gap-2"><Shield size={16} className="text-brand-500" />How to connect your domain</h3>
        <p className="text-sm text-gray-500 mb-4">Go to your domain registrar (GoDaddy, Namecheap, etc.) and add these DNS records:</p>
        ...
        <p className="text-xs text-gray-400 mt-3">DNS changes can take up to 48 hours to propagate.</p>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6"><h2 className="text-xl font-bold">Connect Your Domain</h2><button onClick={() => setShowModal(false)}><X size={20} /></button></div>
            <p className="text-sm text-gray-500 mb-4">Enter the domain you already own. You'll need to update its DNS records to point to our servers.</p>
            <div><label className="input-label">Your Domain</label><input className="input-field" value={domainName} onChange={e => setDomainName(e.target.value)} placeholder="mystore.com" onKeyDown={e => e.key === 'Enter' && addDomain()} /></div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="btn-ghost flex-1">Cancel</button>
              <button onClick={addDomain} disabled={adding} className="btn-primary flex-1 disabled:opacity-50">{adding ? 'Adding...' : 'Connect Domain'}</button>
            </div>
          </div>
        </div>
      )}
